...
2. 运行 `OpenClaw-Setup-0.2.22.exe`
...
- **Control UI（Lit + Electron）：** 在从 GitHub 源码构建 OpenClaw `ui/` 前，由桌面仓库注入 Vite/tsconfig 补丁，使 Lit 字段装饰器按 Electron 内嵌 Chromium 可接受的旧式语义编译；若 npm 包内带有 `dist/control-ui` 会先删除，确保本地与 CI 均用上述配置重建。
...
从[最新发布页](https://github.com/agentkernel/openclaw-desktop/releases/latest)下载 `OpenClaw-Setup-0.2.22.exe` 并运行即可。无需 `npm`、无需系统级 Node.js、无需输入任何终端命令。
</details>
...
**捆绑 OpenClaw：** 在 `package.json` 中通过 `openclawBundleVersion` 固定；执行 `prepare-bundle` 后查看 [`resources/bundle-manifest.json`](resources/bundle-manifest.json) 中的 `bundledOpenClawVersion`（桌面 **v0.2.22** 当前为 **2026.3.23-2**）。本地校验：`pnpm run check-openclaw-versions`（不设 `OPENCLAW_SKIP_NPM_LATEST_CHECK` 时还会与 npm `latest` 对比）。
