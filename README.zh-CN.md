...
2. 运行 `OpenClaw-Setup-0.2.20.exe`
...
- **Electron 41 / Chromium 146：** 提升与上游网关 Control UI 现代 JavaScript 语法的兼容性。
- **网关响应头：** 仅对 HTML 文档响应放宽 CSP，不对脚本等子资源注入合成 CSP。
...
从[最新发布页](https://github.com/agentkernel/openclaw-desktop/releases/latest)下载 `OpenClaw-Setup-0.2.20.exe` 并运行即可。无需 `npm`、无需系统级 Node.js、无需输入任何终端命令。
</details>
...
**捆绑 OpenClaw：** 在 `package.json` 中通过 `openclawBundleVersion` 固定；执行 `prepare-bundle` 后查看 [`resources/bundle-manifest.json`](resources/bundle-manifest.json) 中的 `bundledOpenClawVersion`（桌面 **v0.2.20** 当前为 **2026.3.23-2**）。本地校验：`pnpm run check-openclaw-versions`（不设 `OPENCLAW_SKIP_NPM_LATEST_CHECK` 时还会与 npm `latest` 对比）。
