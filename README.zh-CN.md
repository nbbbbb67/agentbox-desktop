...
- **模型鉴权：** LLM API 保存凭据、启动时迁移 `auth-profiles.json`、读取 `openclaw.json` 时规范化 `auth.order`，均使用完整 profile id（`provider:名称`），与网关解析一致；删除凭据时附带 `provider` 以正确解析 id。

更早版本（v0.3.1）要点：向导 `auth.order`、小米 MiMo 种子、模型预设与 OpenClaw **2026.3.23-2** 对齐 — 详见 [CHANGELOG.md](CHANGELOG.md)。
...
更早版本（v0.2.21）要点：esbuild 后处理 Control UI、稳定通道 `latest.yml` — 详见 [CHANGELOG.md](CHANGELOG.md)。
...
从[最新发布页](https://github.com/agentkernel/openclaw-desktop/releases/latest)下载 `OpenClaw-Setup-0.3.2.exe` 并运行即可。无需 `npm`、无需系统级 Node.js、无需输入任何终端命令。
</details>
...
**捆绑 OpenClaw：** 在 `package.json` 中通过 `openclawBundleVersion` 固定；执行 `prepare-bundle` 后查看 [`resources/bundle-manifest.json`](resources/bundle-manifest.json) 中的 `bundledOpenClawVersion`（桌面 **v0.3.2** 当前为 **2026.3.23-2**）。本地校验：`pnpm run check-openclaw-versions`（不设 `OPENCLAW_SKIP_NPM_LATEST_CHECK` 时还会与 npm `latest` 对比）。
