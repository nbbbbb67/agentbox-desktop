...
2. 运行安装程序（文件名与 `package.json` 一致，例如 `OpenClaw-Setup-0.5.0+openclaw.2026.3.28.exe`）
...
- **Shell 版本：** `0.5.0+openclaw.2026.3.28`（主版本号 + 构建元数据中的捆绑 OpenClaw 版本）。
- **捆绑 OpenClaw（npm）：** **2026.3.28**，与 `npm install openclaw@2026.3.28` 一致；在根目录 [`package.json`](package.json) 中用 `openclawBundleVersion` 固定。
- **桌面端近期要点：** 飞书 `registerFull` 在上游 `dist/auth-profiles-*.js` 布局下仍只注册一次；向导 MiniMax 与上游 **仅 M2.7** 目录一致；Control UI 从 GitHub 标签源码构建以适配 Electron；更多见 [CHANGELOG.md](CHANGELOG.md)（如 **[0.4.11]**）。

### 上游 OpenClaw 2026.3.28（摘要）

完整说明：[openclaw/openclaw **v2026.3.28**](https://github.com/openclaw/openclaw/releases/tag/v2026.3.28)。

**破坏性变更**

- **Qwen：** 移除已弃用的 `qwen-portal-auth`（`portal.qwen.ai` OAuth）；请改用 Model Studio / `openclaw onboard --auth-choice modelstudio-api-key`。
- **配置 / Doctor：** 不再自动迁移约两个月以前的极旧配置；过旧键可能在校验阶段失败而非静默改写。

**主要变更（节选）**

- **xAI：** 捆绑 xAI 走 Responses API；新增 `x_search`；与 Grok / 联网搜索与工具配置对齐，减少手动开关插件。
- **MiniMax：** `image-01` 图像能力；对话模型目录收敛为 **M2.7** 系列（移除旧版 M2 / M2.1 / M2.5 / VL-01）。
- **插件 / 钩子：** `before_tool_call` 支持异步 `requireApproval`（执行审批、Telegram、Discord、`/approve` 等）。
- **CLI / 插件：** `openclaw config schema`；Claude / Codex / Gemini CLI 等走插件面；`gateway run --cli-backend-logs`（旧 `--claude-cli-logs` 为兼容别名）。
- **通道与工具：** Telegram、Discord、WhatsApp、BlueBubbles、Matrix、飞书、Microsoft Teams、Google Chat 等大量修复与改进。
- **Control UI：** 敏感配置默认隐藏；显式「展开再编辑」；避免自动暴露密钥。
...
## 与上游 OpenClaw 的兼容性（捆绑 `2026.3.28`）
...
| | |
|---|---|
| **当前版本** | `v0.5.0`（壳 `0.5.0+openclaw.2026.3.28`） |
| **安装包** | `OpenClaw-Setup-0.5.0+openclaw.2026.3.28.exe`（见 [Releases](https://github.com/agentkernel/openclaw-desktop/releases/latest) 实际资产名） |
...
**捆绑 OpenClaw：** 在 `package.json` 中通过 `openclawBundleVersion` 固定；执行 `prepare-bundle` 后查看 [`resources/bundle-manifest.json`](resources/bundle-manifest.json) 中的 `bundledOpenClawVersion`（桌面 **v0.5.0** 当前为 **2026.3.28**）。本地校验：`pnpm run check-openclaw-versions`（不设 `OPENCLAW_SKIP_NPM_LATEST_CHECK` 时还会与 npm `latest` 对比）。
