...
- **飞书设置** — 在应用内管理飞书凭证、DM 配对和 allowlist（从**设置**、**控制台**或**托盘菜单**进入）
- **桌面端审批流程** — 无需 CLI 即可批准待处理的飞书请求；使用配对码时会调用内置 OpenClaw 运行时作为兜底
- **运行时更新** — 捆绑的 OpenClaw 已更新至 `2026.3.13`
- **Kuae 代理修复** — 当网关继承 HTTPS 代理时，自动为 Kuae 端点合并 `NO_PROXY`，避免 TLS 失败
...
从[最新发布页](https://github.com/agentkernel/openclaw-desktop/releases/latest)下载 `OpenClaw-Setup-0.1.1.exe` 并运行即可。无需 `npm`、无需系统级 Node.js、无需输入任何终端命令。
</details>
